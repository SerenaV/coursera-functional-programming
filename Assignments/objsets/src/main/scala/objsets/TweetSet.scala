...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def descendingByRetweet: TweetList = {
    val most: Tweet = mostRetweeted
    val newSet: TweetSet = remove(most)
...
object GoogleVsApple {
  ...
  def influentialTweets (words : List[String]): TweetSet = TweetReader.allTweets.filter((tweet: Tweet) => {
          def tweetContains(words : List[String]) : Boolean = 
                  if (words.isEmpty) false
                  else tweet.text.contains(words.head) || tweetContains(words.tail)
          tweetContains(words)
  })
  
  lazy val googleTweets: TweetSet = influentialTweets (google)
  lazy val appleTweets: TweetSet = influentialTweets (apple)
